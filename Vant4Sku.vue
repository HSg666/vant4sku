      ...
      <div class="vant4-popup-header" @click="openImagePreview">
        ...
        <div class="vant4-popup-info">
          <p class="vant4-popup-price">
            ...
            <!-- 默认划线价（兜底，默认显示） -->
            <span v-else-if="showOriginPrice && selectedSku?.originPrice" class="vant4-default-origin-price"
              >¥{{ selectedSku.originPrice.toFixed(2) }}</span
            >
          </p>
      ...
      <div class="vant4-sku-body">
        ...
        <div v-for="spec in product.specs" :key="spec.key" class="vant4-spec-group">
          ...
          <div class="vant4-spec-options">
            <van-tag
              ...
              :key="option"
              :color="
                getOptionState(spec.key, option).disabled
                  ? '#f7f8fa'
                  : getOptionState(spec.key, option).selected
                  ? '#fde6e9'
                  : '#f7f8fa'
              "
              :text-color="
        ...
        <div v-if="$slots['select-installment']" class="vant4-installment-group">
          ...
          <van-radio-group v-model="checkedInstallment" direction="horizontal">
            <van-radio
              ...
              icon-size="14"
              shape="square"
              class="vant4-installment-radio"
            >
              <span v-if="item === '0'" style="font-size: 13px">不分期</span>
              <!-- 修复：只有选中有效 SKU 时才计算分期价 -->
              <span v-else-if="selectedSku && selectedSku.price" style="font-size: 13px"
                >{{ (selectedSku.price / item).toFixed(2) }} ✖{{ item }}期</span
              >
              <!-- 未选中时显示占位 -->
        ...
        <div class="vant4-quantity-group">
          ...
          <div class="vant4-quota-group">
            <span class="vant4-quota-text" v-if="showQuotaText"
              >({{ finalMin }}件起售，限购{{ finalMax }}件)&nbsp;&nbsp;</span
            >
        ...
        <van-cell-group>
          ...
          <van-field
            ...
            name="picker"
            :label="selectTitle"
            :placeholder="selectTips"
            @click="showPickerPop = true"
          />
